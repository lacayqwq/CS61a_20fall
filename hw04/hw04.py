...
def balanced(m):
    ...
    assert is_mobile(m)
    l = total_weight(m[1][2])
    r = total_weight(m[2][2])
    if l * m[1][1] == r * m[2][1]: 
        if balanced(m[1][2]) and balanced(m[2][2]): return True
    return False
...
def has_path(t, word):
    ...
    assert is_tree(t)
    if is_leaf(t): 
        if word == label(t): return True
    else:
        if word[0] == label(t): 
            if len(word) == 1: return True
            else: return any([has_path(branch, word[1:]) for branch in branches(t)])
    return False
...
def div_interval(x, y):
    ...
    reciprocal of y."""
    "*** YOUR CODE HERE ***"
    assert not(0 <= upper_bound(y) and 0 >= lower_bound(y))
